...
| — | — | — | Структура .agent/ фиксирована (decisions/tasks/context/rules) |
...
- **Создать структуру `.agent/`**: `rules/`, `decisions/`, `tasks/` (с `backlog/`), `context/`, `archive/` (с `tasks/`, `decisions/`, `checkpoints/`)
...
- Если config.design.adr: создать ADR для каждого ключевого решения → `.agent/decisions/`
- Если config.risk_register: создать `.agent/context/risk-register.md`
- Записать результат в `.agent/context/design-report.md`
...
**Выход:** `.agent/context/design-report.md`, опционально `.agent/decisions/*.md`, `.agent/context/risk-register.md`
...
- Записать результат в `.agent/context/red-team-report.md`
...
- Записать манифест в `.agent/tasks/manifest.json` и `.agent/tasks/manifest.md`
...
**Вход:** analysis-report.md, design-report.md (опционально), `.agent/tasks/manifest.json`, checkpoints.json (decomposition: completed).
...
- Записать результат проверки в `.agent/context/baseline-test-report.log` и `.agent/context/setup-report.log`
- Обновить checkpoints.json: `phases.environment = "completed"`

**Выход:** рабочее окружение + `.agent/context/baseline-test-report.log`
...
- **Архивировать завершённые задачи:**
  - Для каждой задачи со статусом `completed` в `.agent/tasks/manifest.json`:
...
- Организовать артефакты по фиксированной структуре (decisions, tasks, context, rules)
...
Директория `.agent/` — канонический слой метаданных проекта. Все артефакты сессии organised по семантическим группам (правила, решения, задачи, контекст, архив). Это единственная структура — `layer_structure` больше не используется.
...
.agent/
  ...
  decisions/                    # архитектурные решения (справочно)
    index.json                  #   машинночитаемый индекс ADR
    001-технологический-стек.md
    002-архитектурный-паттерн.md
    ...
  tasks/                        # задачи
    manifest.json               #   машинночитаемый манифест задач
    manifest.md                 #   человекочитаемая версия манифеста
    backlog/                    #   задачи вне текущего спринта
  context/                      # контекст проекта (справочно)
    analysis-report.md          #   анализ репозитория
    design-report.md            #   архитектурный дизайн
    risk-register.md            #   реестр допущений
    red-team-report.md          #   (опционально)
    baseline-test-report.log    #   baseline тестов
    setup-report.log            #   отчёт о настройке окружения
    glossary.md                 #   глоссарий предметной области (опционально)
  archive/                      # архив завершённых артефактов
    index.json                  #   мета-индекс архива
    tasks/                      #   детали завершённых задач
    decisions/                  #   заменённые ADR
...
Исполнительный агент всегда начинает с `checkpoints.json` и `session-summary.md`,
затем при необходимости обращается к `decisions/` (ADR для понимания "почему"),
`context/` (детали дизайна и анализа), `tasks/` (что было сделано и что осталось).
